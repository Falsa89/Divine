#!/usr/bin/env python3
"""v108_AUTHORITATIVE_RUNTIME - rollup pack 67."""
import json, os, re, subprocess, sys
from datetime import datetime, timezone
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
SCRIPTS = os.path.join(ROOT, "backend", "scripts")
FINAL_PATH = os.path.join(ROOT, "data", "design", "authoritative_runtime", "v108_authoritative_runtime_final_multirun_suite_result_v1.json")
...
BASE_VERDICT = "MEGA_RELEASE_ACCELERATION_67_v108_AUTHORITATIVE_BATTLE_RUNTIME_STAGING_NO_REWARD_LIVE"
SUB = [
...
def _parse(out):
    ...
    return {"pass": pa, "fail": fa, "miss": mi, "required_fail": int(rr.group(1)) if rr else 0}
def _three():
    runs = []
    for i in range(3):
        ...
        out = (r.stdout or "") + "\n" + (r.stderr or "")
        p = _parse(out)
        if not p: print("FAIL parse on run", i+1); sys.exit(1)
...
def _ensure_final():
    ...
    if in_suite and not os.path.isfile(FINAL_PATH):
        print("[v108_AUTHORITATIVE_RUNTIME ROLLUP] suite-mode skip: final aggregation deferred to standalone")
        return None
    runs = _three()
    p = {r["pass"] for r in runs}; f = {r["fail"] for r in runs}; m = {r["miss"] for r in runs}; rq = {r["required_fail"] for r in runs}
    deterministic = len(p)==1 and len(f)==1 and len(m)==1 and len(rq)==1
    last = runs[-1]
    payload = {
        "pack": BASE_VERDICT, "track": "J", "sentinel": SENTINEL,
        "description": "Final 3-run del master validation suite DOPO l'integrazione del pack v108_AUTHORITATIVE_RUNTIME.",
...
def main():
    ...
    final = _ensure_final()
    if final is None:
        print("[v108_AUTHORITATIVE_RUNTIME ROLLUP] suite-mode summary: all sub-validators PASS; final deferred")
        print(f"v108_AUTHORITATIVE_RUNTIME rollup: {len(results)}/{len(SUB)} PASS (suite mode, J deferred)"); sys.exit(0)
    fr = subprocess.run([sys.executable, os.path.join(SCRIPTS, "validate_v108_authoritative_runtime_final_multirun_suite.py")], capture_output=True, text=True, timeout=60)
    ...
    marker_path = os.path.join(md, "mega_release_acceleration_67_v108_authoritative_runtime_rollup_marker_v1.json")
    payload = {
        "pack": BASE_VERDICT, "type": "v108_authoritative_runtime_rollup_marker", "version": 1,
        "generated_at_utc": datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),
        "public_sync_tag": SENTINEL, "validators_total": len(results)+1,
        "validators_pass": len([x for x in results if x["exit_code"]==0]) + (1 if fr.returncode==0 else 0),
        "verdict_string": verdict, "required_fail_final": final.get("required_fail_final"),
        "miss_final": final.get("miss_final"), "optional_fail_final": opt, "optional_fail_target_max": tmax,
        "under_target_max": opt <= tmax, "deterministic": final.get("deterministic"),
        "rollup_pass_does_not_imply_release_readiness": True,
        "safety": {"fake_PASS": False, "validator_weakening": False, "silent_validator_deletion": False, "release_readiness_claimed": False, "authoritative_live_claim": False, "reward_live_activation": False, "progress_live_activation": False, "battle_engine_formula_rewrite": False},
    }
    open(marker_path,"w").write(json.dumps(payload, indent=2, ensure_ascii=False))
    print(f"Rollup marker saved: {marker_path}")
    print(f"Verdict: {verdict}")
    print(f"v108_AUTHORITATIVE_RUNTIME rollup: {len(results)+1}/{len(SUB)+1} PASS")
    sys.exit(0)
