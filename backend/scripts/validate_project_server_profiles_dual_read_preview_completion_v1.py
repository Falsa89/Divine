...
P = Path('/app/data/design/project_management/project_server_profiles_dual_read_preview_completion_v1.json')
...
def main():
    ...
    assert d['verdict'] == 'TRACK_H_PROJECT_SERVER_PROFILES_DUAL_READ_PREVIEW_COMPLETION_READY'
    assert d['global_verdict'] == 'PROJECT_SERVER_PROFILES_DUAL_READ_PREVIEW_READY'
    ...
    assert md5_of(SP) == d['server_profiles_py_md5_post']
    assert md5_of(ECON) == d['economy_py_md5_post']
    assert md5_of(MENU) == d['menu_md5_post']
    # Frontend file evolved
    assert md5_of(SRV) == d['servers_tsx_md5_post']
    assert d['servers_tsx_md5_pre'] != d['servers_tsx_md5_post']
    # Tracks all READY
    for k in 'ABCDEFGH':
    ...
    # Progress maximum +0.5pp
    diff = d['progress_estimate']['global_project_post'] - d['progress_estimate']['global_project_pre']
    assert 0 <= diff <= 0.5
    print(f"[PASS] DUAL-READ Track H completion \u2014 wiring={d['server_profile_wiring_readiness_post']}%, next={d['recommended_next_pack_primary']}")
    return 0
