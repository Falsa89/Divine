...
# -*- coding: utf-8 -*-
"""
Validator: PROJECT_MATERIAL_RAID_LIVE_CLAIM_SAFETY_HARDENING_PACK (v37 Track B)
Phase: PHASE_7B_MATERIAL_RAID_CLAIM_SAFETY_HARDENING
...
Material Raid live claim. Nessun live claim. Nessuna grant materiali.
Nessuna mutazione user_materials. Nessuna scrittura DB. Nessun consumo di
stamina/tickets/paid attempt.
"""
...
BACKEND_ROUTE_REL = 'backend/routes/material_raid_claim_safety_preview.py'
DESIGN_REL = 'data/design/economy_safety/material_raid_claim_safety_preview_v1.json'
PROOF_REL = 'data/design/economy_safety/material_raid_claim_safety_preview_proof_marker_v1.json'
DOC_REL = 'docs/divine/239_MATERIAL_RAID_LIVE_CLAIM_SAFETY_HARDENING.md'
VALIDATOR_REL = 'backend/scripts/validate_project_material_raid_claim_safety_hardening_v1.py'
SUITE_REL = 'backend/scripts/run_hero_skill_kit_validator_suite.py'
LEGACY_MATERIAL_RAID_PREVIEW_REL = 'backend/routes/material_raid_preview.py'

FEATURE_FLAG = 'MATERIAL_RAID_CLAIM_SAFETY_PREVIEW_ENABLED'
NAMESPACE = '/api/material-raid-claim-safety-preview'
ENDPOINTS = ['/config', '/validate-claim-request', '/grant-plan-preview', '/idempotency-preview']
...
PROOF_REQUIRED = {
    'material_raid_claim_safety_preview_route_created': True,
    ...
    'material_raid_preview_route_changed': False,
...
if not os.path.isfile(repo(LEGACY_MATERIAL_RAID_PREVIEW_REL)):
    fail(f'[2] legacy {LEGACY_MATERIAL_RAID_PREVIEW_REL} must still exist (unchanged)')

if FAILURES:
    for f in FAILURES:
        print('FAIL:', f)
    print('[FAIL] PROJECT_MATERIAL_RAID_CLAIM_SAFETY_HARDENING validator')
    sys.exit(1)
...
# [7] required claim request fields
for fld in REQUIRED_REQUEST_FIELDS:
    if f'"{fld}"' not in route_text:
        fail(f'[7] route must reference required claim request field: {fld}')
...
if design.get('operation_family') != 'material_raid_claim':
    fail('[12] design operation_family must be material_raid_claim')
...
if proof.get('suite_runner_tuple_v37_track_b_count') != 1:
    fail('[13] proof marker suite_runner_tuple_v37_track_b_count must be 1')

# [14] server.py registra il router
sv = read_text('backend/server.py')
if 'from routes.material_raid_claim_safety_preview import router' not in sv:
    fail('[14] server.py must import material_raid_claim_safety_preview router')
if 'material_raid_claim_safety_preview_router' not in sv:
    fail('[14] server.py must register material_raid_claim_safety_preview_router')

# [15] suite runner tuple v37 Track B count = 1
sr = read_text(SUITE_REL)
token = "'PROJECT-MATERIAL-RAID-CLAIM-SAFETY-HARDENING'"
cnt = sr.count(token)
if cnt != 1:
    fail(f'[15] suite runner must contain exactly 1 v37 Track B tuple token, got {cnt}')
val_name = 'validate_project_material_raid_claim_safety_hardening_v1.py'
if sr.count(f"'{val_name}'") != 1:
    fail(f'[15] suite runner must reference {val_name} exactly once')

# [16] no live claim endpoint (no /claim /grant /execute /apply esposti)
banned_endpoints = ['/claim', '/grant', '/execute', '/apply', '/live-claim', '/perform']
for ep in banned_endpoints:
    g = f'@router.get("{ep}")'
    p = f'@router.post("{ep}")'
    if g in route_text or p in route_text:
        fail(f'[16] route must not expose live-claim endpoint: {ep}')
...
print('[PASS] PROJECT_MATERIAL_RAID_CLAIM_SAFETY_HARDENING validator')
sys.exit(0)
