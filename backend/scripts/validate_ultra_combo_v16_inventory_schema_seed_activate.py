#!/usr/bin/env python3
"""ULTRA-COMBO V16 — Composite validator."""
...
if isinstance(status, dict):
    ...
    rec('inv_flag_dep_present', 'inventory_writes_flag_dependency' in status, '')
    ...
    rec('stage1_allowlist_50', status.get('canary_allowlist_size') == 50, '')
    rec('stage1_cap_500', status.get('canary_ledger_cap') == 500, '')
...
rec('live_gift_spend_non_allowlist_423',
    _post('/affinity/gift-spend', {'gift_id':'x','hero_id':'greek_zeus','quantity':1,'idempotency_key':'v16compositek','user_id':'unauth_user_zzz'}) == 423, '')
...
try:
    ...
    rec('db_inv_mut_min_3', inv_mut >= 3, f'count={inv_mut}')
    rec('db_aff_mut_min_3', aff_mut >= 3, f'count={aff_mut}')
    ...
    rec('ugi_seed_50', db['user_gift_inventory'].count_documents({'metadata.seed_task': 'V16'}) == 50, '')
    rec('ugi_no_negative', db['user_gift_inventory'].count_documents({'quantity': {'$lt': 0}}) == 0, '')
except Exception as e:
    rec('db_only_canary_writes', False, f'{e!r}')

# Battle / synergy / catalog files unchanged
try:
    out = subprocess.run(
        ['git','-C','/app','diff','--stat','--',
...
# Shadow adapter not imported live (still inert)
...
print('='*70); print('ULTRA-COMBO V16 — Inventory Schema + Seed + Activate Composite'); print('='*70)
...
OUT = Path('/app/backend/reports/ultra_combo_v16_validator_summary_v1.json')
OUT.parent.mkdir(parents=True, exist_ok=True)
OUT.write_text(json.dumps({
    'combo_id': 'ULTRA_COMBO_V16',
    'task_origin': 'V16-INVENTORY-SCHEMA+SEED+ACTIVATE+LIVE-MONITORING+SAFETY-ROLLUP-K',
    ...
    'safety_flags': {
        ...
        'broad_rollout_authorized': False, 'stage1_applied': True,
