#!/usr/bin/env python3
"""AF2-L-K6-PREP V15 fallback probe — result validator."""
from __future__ import annotations
import json, sys
from pathlib import Path

R = Path('/app/data/design/affinity/af2n_v15_k6_fallback_probe_result_v1.json')
failures=[]; checks=[]
def rec(n, ok, note=''):
...
rec('id', r.get('result_id') == 'af2n_v15_k6_fallback_probe_result_v1', '')
rec('task', r.get('task_origin') == 'AF2-L-K6-LIVE-INSTALL-PREP (fallback)', '')
...
rec('duplicate_zero', r.get('duplicate_inserted_total', 99) == 0, '')
rec('ledger_unchanged', r.get('ledger_row_count_unchanged') is True, '')
rec('rounds_min_100', r.get('rounds_per_label', 0) >= 100, '')
rec('total_requests_min_1000', r.get('total_requests', 0) >= 1000, '')

by = r.get('by_label') or {}
for lbl in ('empty','no_idem','malformed_idem','negative_qty','huge_qty',
            'borea','greek_borea','primordial_gaia','idempotent_replay','stage1_qa_blocked_path'):
    rec(f'label:{lbl}', lbl in by, '')
    if lbl in by:
...
for ep in ('/health','/heroes','/affinity/gifts','/affinity/gift-spend/canary-status',
           '/affinity/gifts/by-element/dark/by-faction/greek',
           '/affinity/gifts/by-element/dark/by-faction/borea',
           '/affinity/gifts/by-element/tides/by-faction/greek'):
...
print('='*70); print('AF2-L-K6-PREP V15 FALLBACK — Validator'); print('='*70)
