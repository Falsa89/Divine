#!/usr/bin/env python3
"""v110 PSP prep rollup pack 70."""
import json, os, re, subprocess, sys
from datetime import datetime, timezone

R = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
S = os.path.join(R, "backend", "scripts")
FINAL = os.path.join(R, "data", "design", "v110_psp_migration", "v110_final_multirun_suite_result_v1.json")
MASTER = os.path.join(S, "run_hero_skill_kit_validator_suite.py")
SENT = "PUBLIC_SYNC_TAG_v110_PSP_APPLY_AND_LEGACY_CLEANUP_PREP_GATED_NOT_APPLIED"
BASE = "MEGA_RELEASE_ACCELERATION_70_v110_PSP_APPLY_AND_LEGACY_CLEANUP_PREP_GATED_NOT_APPLIED"
SUB = [
    "validate_v110_baseline_multirun.py",
    "validate_v110_account_global_vs_server_scoped_matrix.py",
    "validate_v110_psp_schema_and_index_plan.py",
    "validate_v110_backup_manifest_plan.py",
    "validate_v110_psp_dry_run_migration.py",
    "validate_v110_legacy_cleanup_dry_run.py",
    "validate_v110_bot_reconstruction_policy.py",
    "validate_v110_economy_migration_split_plan.py",
    "validate_v110_apply_script_gated_not_executed.py",
    "validate_v110_rollback_script_gated_not_executed.py",
    "validate_v110_server_id_filter_readiness_update.py",
    "validate_v110_zero_mutation_and_gate_preservation.py",
    "validate_v110_runtime_invariant_preservation.py",
]
...
def _ensure_final():
    ...
    if in_suite and not os.path.isfile(FINAL):
        print("[v110 ROLLUP] suite-mode skip")
        return None
    runs = _three()
...
def main():
    ...
    final = _ensure_final()
    if final is None:
        print(f"v110 rollup: {len(results)}/{len(SUB)} PASS (suite mode)")
        sys.exit(0)
    fr = subprocess.run([sys.executable, os.path.join(S, "validate_v110_final_multirun_suite.py")], capture_output=True, text=True, timeout=60)
    ...
    mp = os.path.join(md, "mega_release_acceleration_70_v110_psp_prep_rollup_marker_v1.json")
    payload = {
        "pack": BASE,
        "type": "v110_psp_prep_rollup_marker",
        "version": 1,
    ...
    print(f"v110 rollup: {len(results) + 1}/{len(SUB) + 1} PASS")
    sys.exit(0)
