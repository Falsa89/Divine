#!/usr/bin/env python3
"""V22 — Composite Validator (ULTRA-COMBO V22)."""
...
def main():
    ...
    if _post('/api/affinity/gift-spend', {'gift_id':'x','hero_id':'borea','quantity':1,'idempotency_key':'v22cmp01','user_id':'stage4_qa_001'}) != 404:
    ...
    overall = (len(fails) == 0)
    out = {
        'composite_id':'ultra_combo_v22_stage4_monitoring_redisprep_composite_v1',
        ...
        'safety_invariants': [
            ...
            'rate-limit active', 'Stage4 internal beta active',
            '/api/heroes=100',
        ],
    }
    Path('/app/backend/reports').mkdir(parents=True, exist_ok=True)
    Path('/app/backend/reports/ultra_combo_v22_composite.json').write_text(json.dumps(out, indent=2))
    print(f'ULTRA-COMBO-V22 {out["overall_status"]} fails={len(fails)}')
