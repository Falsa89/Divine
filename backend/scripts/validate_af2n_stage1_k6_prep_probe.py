#!/usr/bin/env python3
"""AF2-L-K6-PREP3 — Probe result validator."""
from __future__ import annotations
import json, sys
from pathlib import Path

R = Path('/app/data/design/affinity/af2n_stage1_k6_prep_probe_result_v1.json')
failures=[]; checks=[]
def rec(n, ok, note=''):
...
rec('id', r.get('result_id') == 'af2n_stage1_k6_prep_probe_result_v1', '')
rec('task', r.get('task_origin') == 'AF2-L-K6-PREP3', '')
...
rec('rounds_min_40', r.get('rounds_per_label', 0) >= 40, '')
rec('total_requests_min_400', r.get('total_requests', 0) >= 400, '')

by = r.get('by_label') or {}
for lbl in ('empty','no_idem','malformed_idem','negative_qty','huge_qty',
            'borea','greek_borea','primordial_gaia','idempotent_replay','stage1_qa_user_blocked_path'):
...
print('='*70); print('AF2-L-K6-PREP3 — Probe Validator'); print('='*70)
