...
d = json.load(open(os.path.join(R, "data/design/v110_psp_migration/v110_runtime_invariant_preservation_v1.json")))
chg = d.get("validator_count_change", {})
assert chg.get("deleted", 0) == 0 and chg.get("silently_deleted", 0) == 0 and chg.get("weakened", 0) == 0
print("[v110 RUNTIME_INVARIANT_PRESERVATION] OK runtime=10 rollups=9 deleted=0 weakened=0")
