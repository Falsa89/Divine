#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validator v80 Track B — Wave2 Files."""
import json, sys
...
def main():
    manifest = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave2_files_manifest_v1.json")
    marker = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave2_files_marker_v1.json")
    if manifest is None: ERR.append("missing:manifest")
    if marker is None: ERR.append("missing:marker")
    for f in ("wave2_allowlist_v1.json", "wave2_reward_fixtures_v1.json", "wave2_plan_v1.json"):
        if not (STAGING / f).exists(): ERR.append(f"missing_wave2_file:{f}")
    allowlist = _load(STAGING / "wave2_allowlist_v1.json")
    if allowlist:
        users = allowlist.get("allowlist", [])
        if not users or len(users) > 3 or any("@" in u for u in users):
            ERR.append("allowlist.invalid_or_pii")
        if allowlist.get("alias_only") is not True: ERR.append("allowlist.alias_only_not_true")
    fixtures = _load(STAGING / "wave2_reward_fixtures_v1.json")
    if fixtures:
    ...
    plan = _load(STAGING / "wave2_plan_v1.json")
    if plan:
        if len(plan.get("plan", [])) != 3: ERR.append("plan.entries_not_3")
    if ERR:
        print("FAIL pve_reward_claim_canary_wave2_files:", "; ".join(ERR)); return 1
    print("PASS pve_reward_claim_canary_wave2_files"); return 0
