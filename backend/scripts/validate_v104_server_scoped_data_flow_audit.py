...
if ui.get('banner_text_required') != 'SERVER_DATA_ISOLATION_BACKEND_PENDING': print('FAIL \u2014 banner_text_required missing token'); sys.exit(1)
...
# Verifica servers.tsx contiene il token banner
...
if 'SERVER_DATA_ISOLATION_BACKEND_PENDING' not in content: print('FAIL \u2014 servers.tsx must contain SERVER_DATA_ISOLATION_BACKEND_PENDING token'); sys.exit(1)
print(f"PASS \u2014 v104 server-scoped data flow audit ({len(surfaces)} surfaces, banner token present)")
sys.exit(0)
