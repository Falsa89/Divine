#!/usr/bin/env python3
"""
RM1.22-J — Controlled Activation of Official 4★ Placeholder Heroes
═══════════════════════════════════════════════════════════════════════

Attiva ESCLUSIVAMENTE i 24 eroi ufficiali 4★ pending (RM1.22-G-BULK
...
`approved_final`. Esclude Hoplite/Berserker/1★/2★/3★ già attivi e
greek_borea (rimane pending premium).

DEFAULT: dry-run (read-only).
APPLY  : richiede env-var
        PLACEHOLDER_4STAR_ACTIVATION_CONFIRM=
          I_UNDERSTAND_THIS_WILL_ACTIVATE_4STAR_PLACEHOLDER_HEROES
        e flag --apply.

Uso:
    # Dry-run
    python backend/scripts/activate_placeholder_4star.py

    # Apply REALE
    PLACEHOLDER_4STAR_ACTIVATION_CONFIRM=I_UNDERSTAND_THIS_WILL_ACTIVATE_4STAR_PLACEHOLDER_HEROES \\
      python backend/scripts/activate_placeholder_4star.py --apply
"""
...
# ═══════════════════════════════════════════════════════════════════════
# 24 official 4★ targets (source of truth:
# data/design/placeholder_assignment_remaining_manifest.json,
# filter native_rarity == 4).
# ═══════════════════════════════════════════════════════════════════════
TARGET_HERO_IDS = [
    "angelic_seraph",
...
ACTIVATION_BATCH = "RM1.22-J_4star_placeholder_activation"
ENV_CONFIRM_KEY = "PLACEHOLDER_4STAR_ACTIVATION_CONFIRM"
ENV_CONFIRM_VAL = "I_UNDERSTAND_THIS_WILL_ACTIVATE_4STAR_PLACEHOLDER_HEROES"
TARGET_NATIVE_RARITY = 4
...
def print_audit(f: dict) -> None:
    print("=" * 78)
    print(" RM1.22-J — PRE-APPLY AUDIT")
    print("=" * 78)
...
async def smoke_visibility(db) -> dict:
    ...
    pending_56_sum = [
        ...
        and (h.get("native_rarity") or 0) >= 5
    ]
    ...
    # Hidden pending 5★/6★ (post-RM1.22-J expectation: 20 + 13 = 33)
    hidden_56 = await db.heroes.count_documents({
        "asset_status": "pending_assets",
        "native_rarity": {"$gte": 5},
    })
    ...
    return {
        ...
        "pending_5_6_summon_eligible": len(pending_56_sum),
        "greek_borea_summon_eligible": len(greek_borea_sum),
        "legacy_borea_summon_eligible": len(legacy_borea_sum),
        "activated_rm122j_4star": activated_targets,
        "hidden_pending_5_6_count": hidden_56,
    }
...
async def main() -> int:
    ap = argparse.ArgumentParser(description="RM1.22-J — Activate 4★ placeholder heroes")
    ...
    report_path = report_dir / f"placeholder_activation_4star_APPLIED_{ts}.json"
    payload = {
        "task": "RM1.22-J",
