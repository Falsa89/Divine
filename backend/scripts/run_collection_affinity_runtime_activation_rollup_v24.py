#!/usr/bin/env python3
"""V29 PART L — Safety Rollup X."""
import json, subprocess, sys, urllib.request
from datetime import datetime, timezone
from pathlib import Path
OUT = Path('/app/data/design/system_safety/collection_affinity_runtime_activation_readiness_rollup_v24.json')
...
def main():
    ...
    rollup = {
        'task_origin': 'AF2-N-V29-SAFETY-ROLLUP-X',
        'version': 'v24',
        ...
        'managed_redis_state': _file_field('/app/data/design/affinity/managed_redis_envaware_v29_result.json'),
        'alerting_state': _file_field('/app/data/design/affinity/alerting_envaware_v29_result.json', key='sink_mode'),
        'schema_fix_regression_status': 'CLOSED' if _file_pass('/app/data/design/affinity/af2n_v28_schema_fix_regression_v29_result.json') else 'OPEN',
        'extended_monitoring_status': 'CLOSED' if _file_pass('/app/data/design/affinity/af2n_scope_s1_extended_monitoring_v29_result.json') else 'OPEN',
        'stress_8x_status': 'CLOSED' if _file_pass('/app/data/design/affinity/af2n_stress_8x_v29_result.json') else 'OPEN',
        'delta_audit_v29_status': 'CLOSED' if _file_pass('/app/data/design/affinity/affinity_inventory_delta_consistency_v29_report.json') else 'OPEN',
        'blocker_matrix_v8_status': 'CLOSED' if _file_pass('/app/data/design/affinity/af2n_broad_rollout_blocker_matrix_v8.json') else 'OPEN',
        'broad_rollout_signoff_v7_status': 'PLAN_READY_NOT_APPROVED' if _file_pass('/app/data/design/affinity/af2n_broad_rollout_signoff_package_v7.json') else 'MISSING',
        'ui_safety_status': 'CLOSED' if _file_pass('/app/data/design/ui/affinity_gifts_public_preview_v29_safety_result.json') else 'OPEN',
        'rollback_readiness_status': 'CLOSED' if _file_pass('/app/data/design/affinity/af2n_v29_rollback_readiness_result_v1.json') else 'OPEN',
    ...
    rollup['verdict'] = 'PASS' if all([
        ...
        rollup['schema_fix_regression_status'] == 'CLOSED',
        rollup['extended_monitoring_status'] == 'CLOSED',
        rollup['stress_8x_status'] == 'CLOSED',
        rollup['delta_audit_v29_status'] == 'CLOSED',
        rollup['blocker_matrix_v8_status'] == 'CLOSED',
        rollup['ui_safety_status'] == 'CLOSED',
        rollup['rollback_readiness_status'] == 'CLOSED',
        rollup['broad_rollout_signoff_v7_status'] == 'PLAN_READY_NOT_APPROVED',
    ]) else 'FAIL'
