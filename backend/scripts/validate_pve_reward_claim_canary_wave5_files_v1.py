#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validator v83 Track B — Wave5 Files."""
import json, sys
...
def main():
    manifest = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave5_files_manifest_v1.json")
    marker = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave5_files_marker_v1.json")
    if manifest is None: ERR.append("missing:manifest")
    if marker is None: ERR.append("missing:marker")
    for f in ("wave5_allowlist_v1.json", "wave5_reward_fixtures_v1.json", "wave5_plan_v1.json"):
        if not (STAGING / f).exists(): ERR.append(f"missing_wave5_file:{f}")
    allowlist = _load(STAGING / "wave5_allowlist_v1.json")
    if allowlist:
        users = allowlist.get("allowlist", [])
        if not users or len(users) > 12 or any("@" in u for u in users):
            ERR.append("allowlist.invalid_or_pii")
        if allowlist.get("alias_only") is not True: ERR.append("allowlist.alias_only_not_true")
        if allowlist.get("max_users") != 12: ERR.append("allowlist.max_users_not_12")
    fixtures = _load(STAGING / "wave5_reward_fixtures_v1.json")
    if fixtures:
    ...
    plan = _load(STAGING / "wave5_plan_v1.json")
    if plan:
        if len(plan.get("plan", [])) != 12: ERR.append("plan.entries_not_12")
        if plan.get("max_users") != 12: ERR.append("plan.max_users_not_12")
        if plan.get("max_claims_total") != 12: ERR.append("plan.max_claims_total_not_12")
    if ERR:
        print("FAIL pve_reward_claim_canary_wave5_files:", "; ".join(ERR)); return 1
    print("PASS pve_reward_claim_canary_wave5_files"); return 0
