#!/usr/bin/env python3
"""Validator: MEGA-RELEASE-ACCELERATION-7-v58-ROLLUP.

Aggregatore Pack v58 multi-mode visual preview shell batch.
Verifica:
 - 5 MD5 invariants ufficiali intatti
 - preferred-unchanged guardrails (server.py, combat.tsx, story.tsx) intatti
 - 6 tuple OPTIONAL nel suite runner (count=1)
 - PUBLIC_SYNC_TAG v58 nel suite runner
 - 5 dependent validators v58 PASS
 - 6 docs (338-343) presenti
 - rollup marker presente e coerente con tutti i flag
...
ROLLUP_MARKER = os.path.join(ROOT, 'data/design/release_acceleration/mega_release_acceleration_7_v58_rollup_marker_v1.json')
PACK = 'MEGA_RELEASE_ACCELERATION_7_MULTI_MODE_VISUAL_PREVIEW_SHELL_BATCH_PACK_v58'
PUBLIC_SYNC_TAG = 'PUBLIC_SYNC_TAG_v58_MEGA_RELEASE_ACCELERATION_7_MULTI_MODE_VISUAL_PREVIEW_SHELL_BATCH'

REQUIRED_TUPLES = [
    "'PROJECT-MULTI-MODE-VISUAL-PREVIEW-SHELL-BATCH-CONTRACT'",
    "'PROJECT-MULTI-MODE-VISUAL-PREVIEW-SCREENS'",
    "'PROJECT-GENERIC-ROUTER-MULTI-MODE-DETAIL'",
    "'PROJECT-MULTI-MODE-VISUAL-PREVIEW-SHELL-BATCH-SMOKE-MATRIX'",
    "'PROJECT-VISUAL-PREVIEW-RUNTIME-SHELL-PROGRESS-REPORT-v3'",
...
DEPENDENT = [
    'validate_multi_mode_visual_preview_shell_batch_contract_v1.py',
    'validate_multi_mode_visual_preview_screens_v1.py',
    'validate_generic_router_multi_mode_detail_v1.py',
    'validate_multi_mode_visual_preview_shell_batch_smoke_matrix_v1.py',
    'validate_visual_preview_runtime_shell_progress_report_v3.py',
]
REQUIRED_DOCS = [
    'docs/divine/338_MULTI_MODE_VISUAL_PREVIEW_BATCH_CONTRACT.md',
    'docs/divine/339_STORY_TOWER_EVENT_ARENA_PREVIEW_SCREENS.md',
    'docs/divine/340_GENERIC_ROUTER_MULTI_MODE_DETAIL_ENHANCEMENT.md',
    'docs/divine/341_MULTI_MODE_VISUAL_PREVIEW_QA_SMOKE.md',
    'docs/divine/342_VISUAL_PREVIEW_RUNTIME_SHELL_PROGRESS_REPORT_v3.md',
    'docs/divine/343_MEGA_RELEASE_ACCELERATION_7_MULTI_MODE_VISUAL_PREVIEW_BATCH_v58.md',
]
...
if not os.path.exists(ROLLUP_MARKER): fail(f'[5] missing rollup marker: {ROLLUP_MARKER}')
    ...
    for k, v in (
        ('marker_version', 'mega_release_acceleration_7_v58_rollup_marker_v1'),
        ('pack', PACK),
        ...
        ('fake_pass', False),
        ('gate_0_v57_pass', True),
        ('gate_1_v58_validators_pass', True),
    ):
    ...
    tr = m.get('multi_mode_batch_transitions') or {}
    for mode in ('story','tower','event','arena'):
        sub = tr.get(mode) or {}
        if sub.get('from') != 'design_only_runtime_deferred' or sub.get('to') != 'preview_shell_v58':
            fail(f'[5] rollup multi_mode_batch_transitions.{mode} mismatch')

if FAILS:
    for f in FAILS: print('FAIL:', f)
    print('[FAIL] MEGA_RELEASE_ACCELERATION_7_v58_ROLLUP validator')
    sys.exit(1)
print('[PASS] MEGA_RELEASE_ACCELERATION_7_v58_ROLLUP validator')
sys.exit(0)
