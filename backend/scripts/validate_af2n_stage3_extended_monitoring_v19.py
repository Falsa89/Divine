#!/usr/bin/env python3
"""Validator for AF2-N-STAGE3-EXTENDED-MONITORING V19."""
from __future__ import annotations
import json, subprocess, sys
from pathlib import Path

RESULT = Path('/app/data/design/affinity/af2n_stage3_extended_monitoring_v19_result.json')


def main():
    if not RESULT.exists():
        r = Path('/app/backend/scripts/run_af2n_stage3_extended_monitoring_v19.py')
        if r.exists(): subprocess.run(['python3', str(r)], timeout=240)
    if not RESULT.exists(): print('missing'); return 1
    d = json.loads(RESULT.read_text())
    failures=[]
    def rec(n,c):
        print(f'  [{"OK" if c else "X"}] {n}')
        if not c: failures.append(n)
    print('='*70); print('AF2-N-STAGE3-EXTENDED-MONITORING V19 — Validator'); print('='*70)
    rec('overall_pass', d.get('overall_status') == 'PASS')
    rec('triggers_zero', d.get('triggers_total') == 0)
    c = d.get('counters', {})
    rec('samples_>=300', c.get('samples_total', 0) >= 300)
    ...
    rec('inv_delta_eq_aff', p.get('inv_mut_delta') == p.get('aff_mut_delta'))
