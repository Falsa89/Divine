#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validator v81 Track D — Wave3 Apply Result + Snapshot + Negative Tests."""
import json, sys
...
def main():
    apply_r = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave3_apply_result_v1.json")
    snap = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave3_ledger_snapshot_v1.json")
    neg = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave3_replay_negative_test_result_v1.json")
    marker = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave3_apply_marker_v1.json")
    ...
    if apply_r:
        ...
        if not isinstance(apply_r.get("wave3_success_count"), int):
            ERR.append("apply_r.wave3_success_count_missing")
        if apply_r.get("applied_to_local_staging") is True:
            if apply_r.get("verdict_local") != "PVE_REWARD_CLAIM_CANARY_WAVE3_OBSERVED_SAFE":
                ERR.append("apply_r.verdict_invalid_when_applied")
            if apply_r.get("wave3_success_count", 0) < 4 or apply_r.get("wave3_success_count", 0) > 5:
                ERR.append("apply_r.wave3_success_count_out_of_range")
    if snap:
    ...
    if ERR:
        print("FAIL pve_reward_claim_canary_wave3_apply:", "; ".join(ERR)); return 1
    print("PASS pve_reward_claim_canary_wave3_apply"); return 0
