#!/usr/bin/env python3
"""V27 PART I — UI safety recheck (same false-positive-aware rules)."""
import json, re, sys
from datetime import datetime, timezone
from pathlib import Path

OUT = Path('/app/data/design/ui/affinity_gifts_public_preview_v27_safety_result.json')
...
CRITICAL_PATTERNS = [
    ...
    (r"BROAD[_\-]?ROLLOUT\s*[:=]\s*['\"]?(?:true|on|enabled)", 'broad rollout flag in UI'),
    (r"PUBLIC[_\-]?SPEND[_\-]?UI\s*[:=]\s*['\"]?(?:true|on|enabled)", 'public spend UI flag'),
        ...
        for f in FRONTEND_DIR.rglob(ext):
            ...
            for pat, desc in CRITICAL_PATTERNS:
                for m in re.finditer(pat, txt, re.IGNORECASE | re.DOTALL):
                    critical.append({
                        'file': str(f.relative_to('/app')), 'pattern': desc,
                        'match': m.group(0)[:120],
                        'line_approx': txt[:m.start()].count('\n') + 1,
                    })
    out = {
        'task_origin': 'AF2-N-V27-UI-PUBLIC-PREVIEW-SAFETY',
        'timestamp_utc': datetime.now(timezone.utc).isoformat(),
        'frontend_files_scanned': files_scanned,
        'critical_findings': critical,
        'critical_count': len(critical),
    }
    out['verdict'] = 'PASS' if not critical else 'FAIL'
    OUT.write_text(json.dumps(out, indent=2, default=str))
    print(f"verdict={out['verdict']} files={files_scanned} critical={len(critical)} → {OUT}")
