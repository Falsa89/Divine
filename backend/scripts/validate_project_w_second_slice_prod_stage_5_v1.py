#!/usr/bin/env python3
# PROJECT_W TRACK C — SECOND SLICE PROD STAGE 5% VALIDATOR
import json, sys, hashlib
from pathlib import Path
MARKER = Path('/app/data/design/status_effects/project_w_second_slice_prod_stage_5_result_v1.json')
...
def main():
    ...
    assert m['target_rollout_percent'] == 5
    ...
    valid = (
        'TRACK_C_SECOND_SLICE_PROD_STAGE_5_ENABLED_SAFE',
        'TRACK_C_SECOND_SLICE_PROD_STAGE_5_READY_NOT_APPLIED_PENDING_APPROVAL',
        'TRACK_C_SECOND_SLICE_PROD_STAGE_5_BLOCKED',
    )
    assert m['verdict'] in valid
    if m['verdict'] == 'TRACK_C_SECOND_SLICE_PROD_STAGE_5_ENABLED_SAFE':
        assert m['escalation_dependency_met'] is True
    print(f'[PASS] PROJECT_W Track C stage 5% — verdict={m["verdict"]}, escalation_dep_met={m["escalation_dependency_met"]}')
    return 0
