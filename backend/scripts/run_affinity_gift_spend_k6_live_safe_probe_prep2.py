#!/usr/bin/env python3
"""AF2-L-K6-LIVE-PREP2 — Extended canary-aware safe probe (Python fallback).

Reuses the AF2-L safe probe pattern but now runs while the AF2-N canary is
ACTIVE. It MUST NOT create any new ledger rows. Therefore it only exercises:
  - non-allowlist user (423)
  - empty/malformed payloads (423)
  - Borea aliases (404)
  - idempotent replay against an existing canary tx (200 idempotent_replay,
    no new row)
  - GET regression endpoints

Writes artifact to /app/data/design/affinity/affinity_gift_spend_k6_live_prep2_result_v1.json
"""
...
RESULT = Path('/app/data/design/affinity/affinity_gift_spend_k6_live_prep2_result_v1.json')
...
def main(argv=None):
    ...
    ap.add_argument('--rounds', type=int, default=80, help='requests per label (default 80)')
    args = ap.parse_args(argv)
    rounds = max(50, min(500, args.rounds))
    ...
    labels = {
        ...
        'negative_qty':    {'body': {'gift_id': 'x', 'hero_id': 'greek_zeus', 'quantity': -3, 'idempotency_key': 'negidemxxx1', 'user_id': 'unauth_user_xxx'}, 'expected': 423},
        'huge_qty':        {'body': {'gift_id': 'x', 'hero_id': 'greek_zeus', 'quantity': 99999, 'idempotency_key': 'hugeidem001', 'user_id': 'unauth_user_xxx'}, 'expected': 423},
    ...
    by_label = {}
    total_requests = 0; total_5xx = 0; unexpected_total = 0
    duplicate_inserted_total = 0

    # Pre + post ledger row counts (must remain identical)
    ...
    summary = {
        'result_id': 'affinity_gift_spend_k6_live_prep2_result_v1',
        'task_origin': 'AF2-L-K6-LIVE-PREP2',
        'design_only': False,
        'runtime_attached': True,
        'runtime_attached_canary_only': True,
        'db_write': False,
        'no_live_spend_outside_canary_replay': True,
        ...
        'mode': 'safe_probe_python_fallback_canary_active',
        ...
        'duplicate_inserted_total': duplicate_inserted_total,
    ...
    ok = (total_5xx == 0 and unexpected_total == 0 and duplicate_inserted_total == 0 and ledger_unchanged)
    print(f'AF2-L-K6-LIVE-PREP2: reqs={total_requests}, 5xx={total_5xx}, unexpected={unexpected_total}, dup_inserted={duplicate_inserted_total}, ledger_unchanged={ledger_unchanged}')
