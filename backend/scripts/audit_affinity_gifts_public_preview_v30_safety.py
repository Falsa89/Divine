#!/usr/bin/env python3
"""V30 PART K — UI safety audit V30."""
import json, re, sys
from datetime import datetime, timezone
from pathlib import Path
OUT = Path('/app/data/design/ui/affinity_gifts_public_preview_v30_safety_result.json')
        ...
        for f in FRONTEND_DIR.rglob(ext):
            ...
            for pat,desc in CRITICAL:
                for m in re.finditer(pat,txt,re.IGNORECASE|re.DOTALL):
                    crit.append({'file':str(f.relative_to('/app')),'pattern':desc,'match':m.group(0)[:120],'line_approx':txt[:m.start()].count('\n')+1})
    out={'task_origin':'AF2-N-V30-UI-PUBLIC-PREVIEW-SAFETY',
         'timestamp_utc':datetime.now(timezone.utc).isoformat(),
         'frontend_files_scanned':n,'critical_findings':crit,'critical_count':len(crit),
         'verdict':'PASS' if not crit else 'FAIL'}
    OUT.write_text(json.dumps(out,indent=2,default=str))
    print(f"verdict={out['verdict']} files={n} critical={len(crit)}")
