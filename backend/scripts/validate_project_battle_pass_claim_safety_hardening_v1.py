...
# -*- coding: utf-8 -*-
"""
Validator: PROJECT_BATTLE_PASS_REWARD_CLAIM_SAFETY_HARDENING_PACK (v40 Track A)
Phase: PHASE_10A_BATTLE_PASS_REWARD_CLAIM_SAFETY_HARDENING
Mode:  REWARD_CLAIM_ECONOMY_SAFETY_HARDENING_PREVIEW_ONLY_NO_LIVE_CLAIM

Asserisce gli invarianti del nuovo route preview-only Battle Pass claim.
Nessun reward grant live. Nessun BP unlock/purchase. Nessuna mutazione
inventory/currency/VIP/shop/IAP. Nessun premium users.gems. Nessun BP
Delta. Zero DB. frontend/app/battlepass.tsx MD5 intoccato (54568b8c...).
frontend/app/vip.tsx MD5 intoccato (45fcc989...).
"""
...
BACKEND_ROUTE_REL = 'backend/routes/battle_pass_claim_safety_preview.py'
PROOF_REL = 'data/design/economy_safety/battle_pass_claim_safety_proof_marker_v1.json'
DOC_REL = 'docs/divine/251_BATTLE_PASS_REWARD_CLAIM_SAFETY_HARDENING.md'
VALIDATOR_REL = 'backend/scripts/validate_project_battle_pass_claim_safety_hardening_v1.py'
SUITE_REL = 'backend/scripts/run_hero_skill_kit_validator_suite.py'
FRONTEND_BP_REL = 'frontend/app/battlepass.tsx'
FRONTEND_BP_MD5 = '54568b8cb75a07033f78ef6593aba839'
FRONTEND_VIP_REL = 'frontend/app/vip.tsx'
FRONTEND_VIP_MD5 = '45fcc9890b6b128c37088bc33aa54caf'
...
REQUIRED_GUARD_CHECKS = [
    ...
    'season_exists', 'season_active_or_claim_grace_window_valid',
    'reward_tier_exists', 'reward_slot_exists', 'battle_pass_track_valid',
    'free_track_claim_allowed_if_unlocked',
    'premium_track_claim_requires_entitlement_but_preview_never_grants',
    'reward_not_already_claimed', 'reward_claim_window_valid',
    'reward_payload_schema_valid',
    'no_premium_currency_consumption', 'no_shop_purchase', 'no_vip_unlock',
...
PROOF_REQUIRED = {
    'battle_pass_claim_safety_preview_route_created': True,
...
for rel in [BACKEND_ROUTE_REL, PROOF_REL, DOC_REL, VALIDATOR_REL, SUITE_REL,
            FRONTEND_BP_REL, FRONTEND_VIP_REL]:
...
if FAILURES:
    for f in FAILURES:
        print('FAIL:', f)
    print('[FAIL] PROJECT_BATTLE_PASS_CLAIM_SAFETY_HARDENING validator')
    sys.exit(1)
...
db_tokens = [
    ...
    'db.user_gear', 'db.user_battle_pass', 'db.server_profiles',
...
if proof.get('suite_runner_tuple_v40_track_a_count') != 1:
    fail('[11] proof marker suite_runner_tuple_v40_track_a_count must be 1')

# [12] server.py registra il router
sv = read_text('backend/server.py')
if 'from routes.battle_pass_claim_safety_preview import router' not in sv:
    fail('[12] server.py must import battle_pass_claim_safety_preview router')
if 'battle_pass_claim_safety_preview_router' not in sv:
    fail('[12] server.py must register battle_pass_claim_safety_preview_router')
if sv.count('app.include_router(battle_pass_claim_safety_preview_router)') != 1:
    fail('[12] server.py include_router(battle_pass_claim_safety_preview_router) count must be 1')

# [13] suite runner tuple v40 Track A count = 1
sr = read_text(SUITE_REL)
token = "'PROJECT-BATTLE-PASS-CLAIM-SAFETY-HARDENING'"
if sr.count(token) != 1:
    fail(f'[13] suite runner must contain exactly 1 v40 Track A tuple token, got {sr.count(token)}')
val_name = 'validate_project_battle_pass_claim_safety_hardening_v1.py'
...
banned_endpoints = ['/commit', '/execute', '/apply', '/live-claim',
                    '/perform', '/claim', '/grant', '/purchase', '/unlock']
...
# [15] frontend battlepass.tsx and vip.tsx MD5 strict
for rel, exp in ((FRONTEND_BP_REL, FRONTEND_BP_MD5), (FRONTEND_VIP_REL, FRONTEND_VIP_MD5)):
    with open(repo(rel), 'rb') as f:
        got = hashlib.md5(f.read()).hexdigest()
    if got != exp:
        fail(f'[15] {rel} MD5 must remain {exp} (got {got})')

# [16] MD5 invariants core 5
INVARIANTS = {
    'backend/battle_engine.py': '151ca35ad3bc35f0a6209cb3744ed440',
    'backend/.env': 'ff60bbb79efa329b71aa8ed351ea89b3',
    'backend/routes/artifacts.py': '893f244d85fd45cbe825996463995293',
    'frontend/app/battlepass.tsx': FRONTEND_BP_MD5,
    'frontend/app/vip.tsx': FRONTEND_VIP_MD5,
}
for rel, exp in INVARIANTS.items():
    p = repo(rel)
    if not os.path.isfile(p):
        fail(f'[16] invariant file missing: {rel}')
        continue
    with open(p, 'rb') as f:
        got = hashlib.md5(f.read()).hexdigest()
    if got != exp:
        fail(f'[16] invariant MD5 mismatch on {rel}: expected {exp}, got {got}')
...
print('[PASS] PROJECT_BATTLE_PASS_CLAIM_SAFETY_HARDENING validator')
sys.exit(0)
