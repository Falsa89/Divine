#!/usr/bin/env python3
"""v102 — Device retest matrix validator."""
import os, sys, json
ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
p = os.path.join(ROOT,'data','design','server_select','v102_device_retest_matrix_v1.json')
...
for i, step in enumerate(matrix):
    if 'action' not in step or 'expected' not in step:
        print(f'FAIL \u2014 matrix step {i} missing action/expected'); sys.exit(1)
...
print(f"PASS \u2014 v102 device retest matrix ({len(matrix)} steps, manual QA required for utente)")
sys.exit(0)
