#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validator v82 Track C — Runner Wave4 Mode."""
import json, sys, re
...
def main():
    ...
    marker = _load(ROOT / "data/design/economy/pve_reward_claim_canary_runner_wave4_marker_v1.json")
    if not runner.exists(): ERR.append("missing:runner")
    else:
        src = runner.read_text(encoding="utf-8")
        for needle in ("--wave4-preflight", "--wave4-apply", "--wave4-observe", "--wave4-rollback-drill",
                       "PVE_REWARD_CLAIM_CANARY_WAVE4", "LOCAL_FILE_STAGING",
                       "VALID_WAVE4_ROUTES", "FORBIDDEN_REWARD_KEYS_WAVE4",
                       "event_arena_ranking_reward", "arena_ranking_reward"):
    ...
    if ERR:
        print("FAIL pve_reward_claim_canary_runner_wave4:", "; ".join(ERR)); return 1
    print("PASS pve_reward_claim_canary_runner_wave4"); return 0
