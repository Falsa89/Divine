...
P = Path('/app/data/design/affinity/managed_redis_envaware_v30_result.json')
def main():
    ...
    if d.get('local_redis_unchanged') is not True: print('FAIL: local_touched'); return 2
    if d.get('status') not in ('READY_NOT_APPLIED_ENV_MISSING','PROBED_CONNECTED','PROBE_CONNECTION_FAILED'):
        print('FAIL: status'); return 2
    print(f"PASS: AF2-N-V30-MANAGED-REDIS-PROBE ({d.get('status')})"); return 0
