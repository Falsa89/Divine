...
PACK_131 = [f'validate_pack_131_{n}.py' for n in ['combat_consumes_real_snapshot_contract',
    'snapshot_to_combat_adapter', 'no_battle_engine_mutation', 'post_battle_preview_safe',
...
def main():
    ...
    results = []
    for gname, scripts in [('PACK 127', PACK_127), ('PACK 128', PACK_128), ('PACK 129', PACK_129), ('PACK 130', PACK_130), ('PACK 131', PACK_131)]:
        print(f'\n--- {gname} ---')
        for s in scripts:
    ...
    suite_report = {'suite': 'PACK_127_128_129_130_131_SAFETY_SUITE', 'status': suite_status, 'backend_up': up,
                    'timestamp_utc': datetime.now(timezone.utc).isoformat(), 'results': results}
    ts = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')
    (REPORTS / f'pack_127_128_129_130_131_safety_suite_{ts}.json').write_text(json.dumps(suite_report, indent=2, ensure_ascii=False), encoding='utf-8')
    (REPORTS / 'pack_127_128_129_130_131_safety_suite_latest.json').write_text(json.dumps(suite_report, indent=2, ensure_ascii=False), encoding='utf-8')
