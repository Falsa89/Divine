#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Validator v82 Track D — Wave4 Apply Result + Snapshot + Negative Tests."""
import json, sys
...
def main():
    apply_r = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave4_apply_result_v1.json")
    snap = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave4_ledger_snapshot_v1.json")
    neg = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave4_replay_negative_test_result_v1.json")
    marker = _load(ROOT / "data/design/economy/pve_reward_claim_canary_wave4_apply_marker_v1.json")
    ...
    if apply_r:
        ...
        if not isinstance(apply_r.get("wave4_success_count"), int):
            ERR.append("apply_r.wave4_success_count_missing")
        if apply_r.get("applied_to_local_staging") is True:
            if apply_r.get("verdict_local") != "PVE_REWARD_CLAIM_CANARY_WAVE4_OBSERVED_SAFE":
                ERR.append("apply_r.verdict_invalid_when_applied")
            if apply_r.get("wave4_success_count", 0) < 6 or apply_r.get("wave4_success_count", 0) > 8:
                ERR.append("apply_r.wave4_success_count_out_of_range")
    if snap:
    ...
    if neg:
        ...
        required = {"duplicate_idempotency_replay", "duplicate_conflicting_hash",
                    "non_allowlisted_user", "premium_reward_reject",
                    "over_cap_reward_reject", "malformed_route_reject",
                    "event_arena_ranking_reward_reject"}
        missing = required - tests_names
        if missing: ERR.append(f"neg.missing_tests:{sorted(missing)}")
    if ERR:
        print("FAIL pve_reward_claim_canary_wave4_apply:", "; ".join(ERR)); return 1
    print("PASS pve_reward_claim_canary_wave4_apply"); return 0
