#!/usr/bin/env python3
"""Validator: MEGA-RELEASE-ACCELERATION-9-v60-ROLLUP.

Aggregatore Pack v60. Verifica MD5 invariants, preferred-unchanged guardrails,
7 tuple OPTIONAL count=1 nel suite + PUBLIC_SYNC_TAG, 6 dependent validators PASS,
7 docs (351-357), rollup marker.
"""
...
ROLLUP_MARKER = os.path.join(ROOT, 'data/design/release_acceleration/mega_release_acceleration_9_v60_rollup_marker_v1.json')
PACK = 'MEGA_RELEASE_ACCELERATION_9_ROUTER_ADAPTER_EVENT_ARENA_LOCAL_TIMELINE_BATCH_PACK_v60'
PUBLIC_SYNC_TAG = 'PUBLIC_SYNC_TAG_v60_MEGA_RELEASE_ACCELERATION_9_ROUTER_ADAPTER_EVENT_ARENA_LOCAL_TIMELINE_BATCH'

REQUIRED_TUPLES = [
    "'PROJECT-VISUAL-BATTLE-RUNNER-ROUTER-ADAPTER-PREVIEW'",
    "'PROJECT-EVENT-LOCAL-DUMMY-SEED-WIRING'",
    "'PROJECT-ARENA-LOCAL-DUMMY-SEED-WIRING'",
    "'PROJECT-LOCAL-TIMELINE-SCHEMA-v3-DELTA'",
    "'PROJECT-ROUTER-ADAPTER-EVENT-ARENA-LOCAL-TIMELINE-SMOKE-MATRIX'",
    "'PROJECT-VISUAL-PREVIEW-RUNTIME-SHELL-PROGRESS-REPORT-v5'",
...
DEPENDENT = [
    'validate_visual_battle_runner_router_adapter_preview_v1.py',
    'validate_event_local_dummy_seed_wiring_v1.py',
    'validate_arena_local_dummy_seed_wiring_v1.py',
    'validate_local_timeline_schema_v3_delta_v1.py',
    'validate_router_adapter_event_arena_local_timeline_smoke_matrix_v1.py',
    'validate_visual_preview_runtime_shell_progress_report_v5.py',
]
REQUIRED_DOCS = [
    'docs/divine/351_VISUAL_BATTLE_RUNNER_ROUTER_ADAPTER_PREVIEW.md',
    'docs/divine/352_EVENT_LOCAL_DUMMY_SEED_WIRING.md',
    'docs/divine/353_ARENA_LOCAL_DUMMY_SEED_WIRING.md',
    'docs/divine/354_LOCAL_TIMELINE_SCHEMA_v3_DELTA.md',
    'docs/divine/355_ROUTER_ADAPTER_EVENT_ARENA_QA_SMOKE.md',
    'docs/divine/356_VISUAL_PREVIEW_RUNTIME_SHELL_PROGRESS_REPORT_v5.md',
    'docs/divine/357_MEGA_RELEASE_ACCELERATION_9_ROUTER_ADAPTER_EVENT_ARENA_LOCAL_TIMELINE_BATCH_v60.md',
]
...
if not os.path.exists(ROLLUP_MARKER): fail(f'[5] missing rollup marker: {ROLLUP_MARKER}')
    ...
    for k, v in (
        ('marker_version', 'mega_release_acceleration_9_v60_rollup_marker_v1'),
        ...
        ('gate_0_v59_pass', True),('gate_1_v60_validators_pass', True),
    ...
    for rel, expected in MD5_INVARIANTS.items():
        if md5_block.get(rel) != expected:
            fail(f'[5] rollup md5_invariants {rel} != {expected}')
    st = m.get('state_transitions') or {}
    ev = st.get('event') or {}
    if ev.get('from') != 'preview_shell_v58' or ev.get('to') != 'local_dummy_seed_wired_v60':
        fail('[5] rollup state_transitions.event mismatch')
    ar = st.get('arena') or {}
    if ar.get('from') != 'preview_shell_v58' or ar.get('to') != 'local_dummy_seed_wired_v60':
        fail('[5] rollup state_transitions.arena mismatch')
    if m.get('excluded_lanes') != ['material_raid_claim_safety_hardening_v2_preview_only']:
        fail('[5] rollup excluded_lanes mismatch')

if FAILS:
    for f in FAILS: print('FAIL:', f)
    print('[FAIL] MEGA_RELEASE_ACCELERATION_9_v60_ROLLUP validator')
    sys.exit(1)
print('[PASS] MEGA_RELEASE_ACCELERATION_9_v60_ROLLUP validator')
sys.exit(0)
