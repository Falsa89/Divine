...
d = json.load(open(os.path.join(R, 'data/design/v110_pack_86_lobby_psp_ensure/v110_pack_86_final_multirun_suite_result_v1.json')))
for k in ('final_post_pack_run1','final_post_pack_run2','final_post_pack_run3'):
    r = d.get(k); assert r is not None, f'{k} missing'
    # Allow null only when placeholder is True (skipped during runtime check)
...
if d.get('placeholder_pending_runtime_population') is True:
    print('[v110 PACK_86_FINAL_MULTIRUN_SUITE] OK placeholder_pending_runtime_population (will be populated by final 3-run)')
else:
    ...
    print(f'[v110 PACK_86_FINAL_MULTIRUN_SUITE] OK run1=run2=run3={p1} required=0 miss=0')
