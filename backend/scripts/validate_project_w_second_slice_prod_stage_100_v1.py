#!/usr/bin/env python3
# PROJECT_W TRACK E — SECOND SLICE PROD STAGE 100% VALIDATOR
import json, sys, hashlib
from pathlib import Path
MARKER = Path('/app/data/design/status_effects/project_w_second_slice_prod_stage_100_result_v1.json')
...
def main():
    ...
    assert m['target_rollout_percent'] == 100
    ...
    valid = (
        'TRACK_E_SECOND_SLICE_PROD_STAGE_100_ENABLED_SAFE',
        'TRACK_E_SECOND_SLICE_PROD_STAGE_100_READY_NOT_APPLIED_PENDING_APPROVAL',
        'TRACK_E_SECOND_SLICE_PROD_STAGE_100_BLOCKED',
    )
    assert m['verdict'] in valid
    if m['verdict'] == 'TRACK_E_SECOND_SLICE_PROD_STAGE_100_ENABLED_SAFE':
        assert m['escalation_dependency_met'] is True
    print(f'[PASS] PROJECT_W Track E stage 100% — verdict={m["verdict"]}, escalation_dep_met={m["escalation_dependency_met"]}')
    return 0
