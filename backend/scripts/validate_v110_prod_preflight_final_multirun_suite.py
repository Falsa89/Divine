#!/usr/bin/env python3
# Pack 76 Track M: final 3-run suite deterministica.
import json, os, sys
R = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
F = os.path.join(R, "data/design/v110_prod_preflight/v110_prod_preflight_final_multirun_suite_result_v1.json")
if os.environ.get("SUITE_RUNNER_ACTIVE") == "1" and not os.path.isfile(F):
    print("[v110 PROD_PREFLIGHT_FINAL_MULTIRUN_SUITE] suite-mode skip (artefatto ancora non generato)")
    sys.exit(0)
d = json.load(open(F))
...
print(f"[v110 PROD_PREFLIGHT_FINAL_MULTIRUN_SUITE] OK 3-run deterministic pass={d.get('pass_final')} fail={d.get('fail_final')} opt={opt}")
