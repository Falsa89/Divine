#!/usr/bin/env python3
"""
RM1.22-L — Controlled Activation of Official 6★ Launch-Base Placeholder Heroes
═══════════════════════════════════════════════════════════════════════════════

Attiva ESCLUSIVAMENTE i 12 eroi ufficiali 6★ del release_group "launch_base"
(RM1.22-G-BULK placeholder assets + RM1.22-G-BULK frontend contracts),
impostando i flag DB di visibilità e obtainability senza mai marcarli come
`approved_final`. Esclude Hoplite/Berserker/1★/2★/3★/4★/5★ già attivi,
`greek_borea` (rimane pending extra_premium per banner dedicato futuro) e
legacy `borea`.

DEFAULT: dry-run (read-only).
APPLY  : richiede env-var
        PLACEHOLDER_6STAR_ACTIVATION_CONFIRM=
          I_UNDERSTAND_THIS_WILL_ACTIVATE_6STAR_LAUNCHBASE_PLACEHOLDER_HEROES
        e flag --apply.

Uso:
    # Dry-run
    python backend/scripts/activate_placeholder_6star_launchbase.py

    # Apply REALE
    PLACEHOLDER_6STAR_ACTIVATION_CONFIRM=I_UNDERSTAND_THIS_WILL_ACTIVATE_6STAR_LAUNCHBASE_PLACEHOLDER_HEROES \\
      python backend/scripts/activate_placeholder_6star_launchbase.py --apply
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# 12 official 6★ launch_base targets (source of truth:
# data/design/placeholder_assignment_remaining_manifest.json,
# filter native_rarity == 6 AND release_group == "launch_base").
# `greek_borea` è release_group="launch_extra_premium" → ESCLUSA.
# ═══════════════════════════════════════════════════════════════════════════════
TARGET_HERO_IDS = [
    "greek_athena",
    "mesopotamian_tiamat",
...
ACTIVATION_BATCH = "RM1.22-L_6star_launchbase_placeholder_activation"
ENV_CONFIRM_KEY = "PLACEHOLDER_6STAR_ACTIVATION_CONFIRM"
ENV_CONFIRM_VAL = "I_UNDERSTAND_THIS_WILL_ACTIVATE_6STAR_LAUNCHBASE_PLACEHOLDER_HEROES"
TARGET_NATIVE_RARITY = 6
TARGET_RELEASE_GROUP = "launch_base"
...
# Include: Hoplite/Berserker (3★ active), greek_borea (pending extra_premium,
# banner futuro), legacy borea (deprecated placeholder).
PROTECTED_IDS = {"greek_hoplite", "norse_berserker", "greek_borea", "borea"}
...
async def audit(db) -> dict:
    ...
    try:
        ...
        manifest_set = {
            t["hero_id"] for t in mf["targets"]
            if t.get("native_rarity") == TARGET_NATIVE_RARITY
            and t.get("release_group") == TARGET_RELEASE_GROUP
        }
        f["manifest_match"] = (manifest_set == set(TARGET_HERO_IDS))
        f["manifest_target_count"] = len(manifest_set)
        f["manifest_target_ids"] = sorted(manifest_set)
    ...
    for h in found:
        ...
        if h.get("release_group") != TARGET_RELEASE_GROUP:
            bad.append(f"release_group={h.get('release_group')!r} (expected {TARGET_RELEASE_GROUP!r})")
        if bad:
    ...
    fe_src_path = APP_ROOT / "frontend" / "components" / "ui" / "placeholderHeroContractsRemaining.ts"
    fe_src = fe_src_path.read_text()
    m_con = re.search(
    ...
    # Borea / legacy borea audit (deve restare hidden)
    gb = await db.heroes.find_one({"id": "greek_borea"}, {"image_base64": 0})
    if gb:
        f["greek_borea_audit"] = {
            "exists": True,
            "is_official": gb.get("is_official"),
            "is_legacy_placeholder": gb.get("is_legacy_placeholder"),
            "release_group": gb.get("release_group"),
            "asset_status": gb.get("asset_status"),
            "obtainable": gb.get("obtainable"),
            "show_in_catalog": gb.get("show_in_catalog"),
            "show_in_summon": gb.get("show_in_summon"),
        }
    else:
        f["greek_borea_audit"] = {"exists": False}

    lb = await db.heroes.find_one({"id": "borea"}, {"image_base64": 0})
    if lb:
        f["legacy_borea_audit"] = {
            "exists": True,
            "is_official": lb.get("is_official"),
            "is_legacy_placeholder": lb.get("is_legacy_placeholder"),
            "obtainable": lb.get("obtainable"),
            "show_in_catalog": lb.get("show_in_catalog"),
            "show_in_summon": lb.get("show_in_summon"),
        }
    else:
        f["legacy_borea_audit"] = {"exists": False}

    return f


def audit_passed(f: dict) -> bool:
    gb = f.get("greek_borea_audit", {})
    lb = f.get("legacy_borea_audit", {})
    gb_ok = (
        gb.get("exists") is True
        and gb.get("is_official") is True
        and gb.get("is_legacy_placeholder") is False
        and gb.get("release_group") == "launch_extra_premium"
        and gb.get("asset_status") == "pending_assets"
        and gb.get("obtainable") is False
        and gb.get("show_in_catalog") is False
        and gb.get("show_in_summon") is False
    )
    lb_ok = (
        lb.get("exists") is True
        and lb.get("is_official") is False
        and lb.get("is_legacy_placeholder") is True
        and lb.get("obtainable") is False
        and lb.get("show_in_catalog") is False
        and lb.get("show_in_summon") is False
    )
    return (
...
def print_audit(f: dict) -> None:
    print("=" * 78)
    print(" RM1.22-L — PRE-APPLY AUDIT (6★ launch_base)")
    print("=" * 78)
    print(f"  target heroes count         : {f['target_count']}")
    print(f"  manifest match              : {'OK' if f.get('manifest_match') else 'FAIL'}  (manifest_target_count={f.get('manifest_target_count')})")
    ...
    print()
    print("  BOREA EXCLUSION AUDIT:")
    gb = f.get("greek_borea_audit", {})
    lb = f.get("legacy_borea_audit", {})
    print(f"    greek_borea (extra_premium, must remain hidden): {gb}")
    print(f"    legacy borea (must stay non-official, hidden)  : {lb}")
    print("=" * 78)
...
async def apply_activation(db, dry_run: bool) -> dict:
    ...
    #   - ID ∈ TARGET (12 launch_base 6★)
    #   - is_official True (no legacy)
    #   - is_legacy_placeholder False
    #   - native_rarity == 6
    #   - release_group == "launch_base" (esclude greek_borea extra_premium)
    ...
    flt = {
        ...
        "release_group": TARGET_RELEASE_GROUP,
...
async def smoke_visibility(db) -> dict:
    ...
    return {
        ...
        "activated_rm122l_6star_launchbase": activated_targets,
        "hidden_pending_6_count": hidden_6,
    }


async def verify_protected_untouched(db) -> dict:
    """Legge Hoplite/Berserker/greek_borea/borea per conferma no batch-tag."""
    out = {}
    for hid in sorted(PROTECTED_IDS):
        ...
        out[hid] = {
            ...
            "is_official": h.get("is_official") if h else None,
            "is_legacy_placeholder": h.get("is_legacy_placeholder") if h else None,
            "obtainable": h.get("obtainable") if h else None,
            "show_in_catalog": h.get("show_in_catalog") if h else None,
            "show_in_summon": h.get("show_in_summon") if h else None,
            "asset_status": h.get("asset_status") if h else None,
            "release_group": h.get("release_group") if h else None,
        }
    return out
...
async def main() -> int:
    ap = argparse.ArgumentParser(
        description="RM1.22-L — Activate 6★ launch_base placeholder heroes"
    )
    ap.add_argument(
    ...
    print("\n" + "=" * 78)
    print(" PROTECTED IDs POST-APPLY (Hoplite/Berserker/greek_borea/borea)")
    print("=" * 78)
    ...
    report_path = report_dir / f"placeholder_activation_6star_launchbase_APPLIED_{ts}.json"
    payload = {
        "task": "RM1.22-L",
