#!/usr/bin/env python3
"""
RM1.22-I — Controlled Activation of Official 3★ Placeholder Heroes
═══════════════════════════════════════════════════════════════════════

Attiva ESCLUSIVAMENTE i 22 eroi ufficiali 3★ pending (RM1.22-G-BULK
...
`approved_final`. Esclude Hoplite/Berserker (già attivi) e greek_borea
(rimane pending premium).

DEFAULT: dry-run (read-only).
APPLY  : richiede env-var
        PLACEHOLDER_3STAR_ACTIVATION_CONFIRM=
          I_UNDERSTAND_THIS_WILL_ACTIVATE_3STAR_PLACEHOLDER_HEROES
        e flag --apply.

Uso:
    # Dry-run
    python backend/scripts/activate_placeholder_3star.py

    # Apply REALE
    PLACEHOLDER_3STAR_ACTIVATION_CONFIRM=I_UNDERSTAND_THIS_WILL_ACTIVATE_3STAR_PLACEHOLDER_HEROES \\
      python backend/scripts/activate_placeholder_3star.py --apply
"""
...
# ═══════════════════════════════════════════════════════════════════════
# 22 official 3★ targets (source of truth:
# data/design/placeholder_assignment_remaining_manifest.json,
# filter native_rarity == 3, excluding greek_hoplite + norse_berserker).
# ═══════════════════════════════════════════════════════════════════════
TARGET_HERO_IDS = [
    "celtic_archer",
    "angelic_priestess",
...
ACTIVATION_BATCH = "RM1.22-I_3star_placeholder_activation"
ENV_CONFIRM_KEY = "PLACEHOLDER_3STAR_ACTIVATION_CONFIRM"
ENV_CONFIRM_VAL = "I_UNDERSTAND_THIS_WILL_ACTIVATE_3STAR_PLACEHOLDER_HEROES"
...
async def audit(db) -> dict:
    ...
    try:
        ...
        manifest_3star = {t["hero_id"] for t in mf["targets"] if t.get("native_rarity") == 3}
        f["manifest_match"] = (manifest_3star == set(TARGET_HERO_IDS))
        f["manifest_3star_count"] = len(manifest_3star)
    ...
    for h in found:
        ...
        if h.get("native_rarity") != 3:
            bad.append(f"native_rarity={h.get('native_rarity')!r} (expected 3)")
        if bad:
...
def print_audit(f: dict) -> None:
    print("=" * 78)
    print(" RM1.22-I — PRE-APPLY AUDIT")
    print("=" * 78)
...
async def smoke_visibility(db) -> dict:
    ...
    pending_456_sum = [
        ...
        and (h.get("native_rarity") or 0) >= 4
    ]
    ...
    # Hidden pending 4★/5★/6★ (pending_456)
    hidden_456 = await db.heroes.count_documents({
        "asset_status": "pending_assets",
        "native_rarity": {"$gte": 4},
    })
    ...
    return {
        ...
        "pending_4_5_6_summon_eligible": len(pending_456_sum),
        "greek_borea_summon_eligible": len(greek_borea_sum),
        "legacy_borea_summon_eligible": len(legacy_borea_sum),
        "activated_rm122i_3star": activated_targets,
        "hidden_pending_4_5_6_count": hidden_456,
    }
...
async def main() -> int:
    ap = argparse.ArgumentParser(description="RM1.22-I — Activate 3★ placeholder heroes")
    ...
    report_path = report_dir / f"placeholder_activation_3star_APPLIED_{ts}.json"
    payload = {
        "task": "RM1.22-I",
