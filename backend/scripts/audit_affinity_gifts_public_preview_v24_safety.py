...
OUT = Path('/app/data/design/ui/affinity_gifts_public_preview_v24_safety_result.json')
...
def main():
    ...
    checks['no_put_patch_delete'] = all(f"method: '{m}'" not in stripped and f'method: "{m}"' not in stripped for m in ('PUT','PATCH','DELETE'))
    ...
    checks['no_metrics_admin_endpoint_in_ui'] = '_admin/metrics' not in stripped
    ...
    out_doc = {
        'result_id':'affinity_gifts_public_preview_v24_safety_result',
        'task_origin':'V24-AF2N-PUBLIC-UI-SAFETY-RECHECK',
    ...
    if not overall:
        ...
        return 2
    print('PASS: AF2-N-PUBLIC-UI-V24-SAFETY'); return 0
