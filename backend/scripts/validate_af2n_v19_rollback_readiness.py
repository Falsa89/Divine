#!/usr/bin/env python3
"""Validator for V19 rollback readiness."""
from __future__ import annotations
import json, subprocess, sys
from pathlib import Path

RESULT = Path('/app/data/design/affinity/af2n_v19_rollback_readiness_result_v1.json')


def main():
    if not RESULT.exists():
        r = Path('/app/backend/scripts/run_af2n_v19_rollback_readiness.py')
        if r.exists(): subprocess.run(['python3', str(r)], timeout=60)
    failures=[]
    def rec(n,c):
        print(f'  [{"OK" if c else "X"}] {n}')
        if not c: failures.append(n)
    print('='*70); print('V19 ROLLBACK READINESS — Validator'); print('='*70)
    ...
    rec('ui_preview_present', d.get('ui_preview_present') is True)
    rec('ui_preview_rollback_strategy_present', bool(d.get('ui_preview_rollback_strategy')))
