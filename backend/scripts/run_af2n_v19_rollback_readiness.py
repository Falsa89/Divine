#!/usr/bin/env python3
"""V19 Rollback Readiness — includes UI preview rollback verification."""
from __future__ import annotations
import json, subprocess, sys
from datetime import datetime, timezone
from pathlib import Path

OUT = Path('/app/data/design/affinity/af2n_v19_rollback_readiness_result_v1.json')
ROLLBACK_SCRIPTS = [
...
UI_PREVIEW_FILE = Path('/app/frontend/app/affinity-gifts-preview.tsx')


def main():
    payload = {
        'result_id': 'af2n_v19_rollback_readiness_result_v1',
        'task_origin': 'AF2-N-V19-ROLLBACK-READINESS',
    ...
    # UI preview rollback strategy (delete file)
    payload['ui_preview_present'] = UI_PREVIEW_FILE.exists()
    payload['ui_preview_rollback_strategy'] = 'delete /app/frontend/app/affinity-gifts-preview.tsx + rebuild'
    payload['ui_preview_rollback_command'] = 'rm /app/frontend/app/affinity-gifts-preview.tsx'
    # Backup dir writable
    backup_dir = Path('/app/ops/backups')
    try:
        backup_dir.mkdir(parents=True, exist_ok=True)
        probe = backup_dir / '.readiness_probe_v19'; probe.write_text('ok'); probe.unlink()
    ...
    print(f'V19 rollback readiness: {payload["overall_status"]} (scripts={len(ROLLBACK_SCRIPTS)} all_present={all_present})')
