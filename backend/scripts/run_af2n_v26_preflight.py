#!/usr/bin/env python3
"""V26 PART A — Preflight result generator + validator."""
import json, subprocess, sys, urllib.request
from datetime import datetime, timezone
from pathlib import Path

OUT = Path('/app/data/design/affinity/af2n_v26_preflight_result_v1.json')
...
def _post404(hero):
    body = json.dumps({'gift_id': 'x', 'hero_id': hero, 'quantity': 1,
                       'idempotency_key': f'v26_pre_{hero}', 'user_id': 'stage4_qa_001'}).encode()
...
def main():
    ...
    ui_code, _ = _get('/affinity-gifts-preview')  # via expo at port 3000 - not via backend.
    # Fallback: just check via expo-proxied through port 3000 if available; otherwise skip.
    ...
    # V25 deliverables existence
    v25_deliverables = {
        'support_runbook': Path('/app/docs/divine/85_AF2N_STAGE4_SUPPORT_RUNBOOK_V25.md').exists(),
        'blocker_matrix_v4': Path('/app/data/design/affinity/af2n_broad_rollout_blocker_matrix_v4.json').exists(),
        'alerting_contract_v25': Path('/app/data/design/affinity/af2n_fail_open_alerting_contract_v1.json').exists(),
        'redis_recovery_script': Path('/app/ops/ensure_redis_rate_limit.sh').exists(),
    }

    out = {
        'task_origin': 'AF2-N-V26-PREFLIGHT',
        ...
        'checks': {
            ...
            'v25_deliverables_present': v25_deliverables,
            'v25_all_present': all(v25_deliverables.values()),
        },
    }
